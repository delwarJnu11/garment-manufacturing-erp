...
class TransactionController extends Controller{
	...
	public function create(){
		$voucher_ref= Transaction::latest('id')->value('voucher_ref');
		if ($voucher_ref) {
			// Extract the number from the string
			$number = (int) filter_var($voucher_ref, FILTER_SANITIZE_NUMBER_INT);
			// Increment the number
			$newNumber = $number + 1;
			// Rebuild voucher_ref (keeping "TRX" + new number)
			$newVoucherRef = 'TRX' . $newNumber;
		}
		return view("pages.accounts.transactions.create",["accounts"=>Account::all(),"users"=>User::all(), "voucher_ref"=>$newVoucherRef]);
		// echo $voucher_ref;
	}
